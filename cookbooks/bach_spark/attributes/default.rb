...
default[:spark][:kerberos][:enabled] = false
default[:spark][:hdfs_url] = node['bcpc']['hadoop']['hdfs_url']
default[:spark][:install][:dir] = "#{node[:spark][:package][:base]}/#{node[:spark][:package][:version]}"
default[:spark][:history][:user] = 'bach_nobody'
default[:spark][:history][:principal] = 'bach_nobody@REALM'
default[:spark][:history][:keytab] = '/etc/security/keytabs/bach_nobody'
default[:spark][:history][:dir] = '/var/log/spark-history'
default[:spark][:history][:path] = :File.join(node[:spark][:hdfs_url], node[:spark][:history_dir])
default[:spark][:env][:log_dir] = '/var/log/spark'
default[:spark][:env][:pid_dir] = '/var/run/spark'
default[:spark][:env][:history_opts] = ''
default[:spark][:env][:history_opts] = "-Dspark.history.kerberos.enabled=true " + \
  "-Dspark.history.kerberos.principal=#{node[:spark][:history][:principal]} " + \
  "-Dspark.history.kerberos.keytab=#{node[:spark][:history][:keytab]}" if node[:spark][:kerberos][:enabled]
...
default.bach_spark.config.spark.history.fs.logDirectory = node[:spark][:history][:path]
default.bach_spark.config.spark.eventLog.dir = node[:spark][:history][:path]
...
default.bach_spark.environment.SPARK_LOG_DIR = node[:spark][:env][:log_dir]
default.bach_spark.environment.SPARK_PID_DIR = node[:spark][:env][:pid_dir]
default.bach_spark.environment.SPARK_HISTORY_OPTS =
