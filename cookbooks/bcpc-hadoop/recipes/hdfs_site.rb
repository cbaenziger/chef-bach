...
if node[:bcpc][:hadoop][:kerberos][:enable]
  ...
  spnego_host = if kerberos_data[:spnego][:princhost] == '_HOST'
                  '_HOST'
                else
                  kerberos_data[:spnego][:princhost]
                end
  ...
  namenode_host = if kerberos_data[:namenode][:princhost] == '_HOST'
                    '_HOST'
                  else
                    kerberos_data[:namenode][:princhost]
                  end
  ...
  if node.run_list.expand(node.chef_environment).recipes
    ...
    jn_host = if kerberos_data[:journalnode][:princhost] == '_HOST'
                '_HOST'
              else
                kerberos_data[:journalnode][:princhost]
              end
