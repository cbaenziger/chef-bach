...
node.override[:hannibal][:mysql][:url] =
  "\"jdbc:mysql://#{node[:bcpc][:management][:vip]}/" \
  'hannibal?characterEncoding=UTF-8"'
node.override[:hannibal][:app_secret_key] =
  get_config('hannibal-app-secret-key')
...
if 'mysql' == node[:hannibal][:db]
  # Create DB, User and configure permissions for hannibal
  grant_string = "GRANT ALL ON #{node[:hannibal][:mysql][:db_name]}.* TO " \
                 "'%{user}'@'%{host}' IDENTIFIED BY " \
                 "'#{get_config('hannibal-db-password')}';"
  ruby_block 'hannibal-database-creation' do
    block do
      mysql_root_password = get_config!('password', 'mysql-root', 'os')
      puts %x( mysql -u#{get_config('mysql-root-user')} \
                     -p#{mysql_root_password} \
                     -e "CREATE DATABASE "\
                        "#{node[:hannibal][:mysql][:db_name]} "\
                        "CHARACTER SET UTF8;"
               mysql -u#{get_config('mysql-root-user')} \
                     -p#{mysql_root_password} \
                     -e #{grant_string %
                          { user: get_config('hannibal-db-user'),
                            host: '%' }}
               mysql -u#{get_config('mysql-root-user')} \
                     -p#{mysql_root_password} \
                     -e #{grant_string %
    ...
    not_if "mysql -u#{get_config('mysql-root-user')} \
                  -p#{get_config!('password', 'mysql-root', 'os')} \
                  -e \"SELECT SCHEMA_NAME FROM \" \
                     \"INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = \" \
                     \"\'#{node[:hannibal][:mysql][:db_name]}\'\" | \
            grep #{node[:hannibal][:mysql][:db_name]}"
  end
end

node.override[:hannibal][:zookeeper_quorum] =
  node[:bcpc][:hadoop][:zookeeper][:servers]
node.override[:hannibal][:hbase_rs][:info_port] = 60_300

if node[:bcpc][:hadoop][:kerberos][:enable] == true
  node.override[:hannibal][:hbase_site_xml_additions] =
