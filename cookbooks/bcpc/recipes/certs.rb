#
# Cookbook Name:: bcpc
# Recipe:: certs
#
# Copyright 2013, Bloomberg Finance L.P.
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
# http://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#

require 'openssl'
include_recipe "bcpc::default"

template "/tmp/openssl.cnf" do
    source "openssl.cnf.erb"
    owner "root"
    group "root"
    mode 00644
end

node.run_state[:bcpc] = ( node.run_state[:bcpc] or {} )
node.run_state[:bcpc][:cert_temp] = ""
bootstrap = get_all_nodes.select{|s| s.hostname.include? node[:bcpc][:bootstrap][:hostpattern]}[0].fqdn
key = OpenSSL::PKey::RSA.new 2048;

results = get_nodes_for("certs").map!{ |x| x['fqdn'] }.join(",")
nodes = results == "" ? node['fqdn'] : results

ruby_block "initialize-ssh-keys" do
    block do
        require 'net/ssh'
        pubkey = "#{key.ssh_type} #{[ key.to_blob ].pack('m0')}"
        make_config('ssh-public-key', pubkey)
        if get_config('ssl-certificate').nil? && get_config('certificate','ssl','os').nil? then
            puts "openssl req -config /tmp/openssl.cnf -extensions v3_req -new -x509 -passout pass:temp_passwd -newkey rsa:4096 -out /dev/stdout -keyout /dev/stdout -days 1095 -subj \"/C=#{node['bcpc']['country']}/ST=#{node['bcpc']['state']}/L=#{node['bcpc']['location']}/O=#{node['bcpc']['organization']}/OU=#{node['bcpc']['region_name']}/CN=#{node['bcpc']['domain_name']}/emailAddress=#{node['bcpc']['admin_email']}\""
            node.run_state[:bcpc][:cert_temp] = %x[openssl req -config /tmp/openssl.cnf -extensions v3_req -new -x509 -passout pass:temp_passwd -newkey rsa:4096 -out /dev/stdout -keyout /dev/stdout -days 1095 -subj "/C=#{node['bcpc']['country']}/ST=#{node['bcpc']['state']}/L=#{node['bcpc']['location']}/O=#{node['bcpc']['organization']}/OU=#{node['bcpc']['region_name']}/CN=#{node['bcpc']['domain_name']}/emailAddress=#{node['bcpc']['admin_email']}"]
        end
    end
end

ssh_private_key = get_config("ssh-private-key")
if ssh_private_key.nil?
  ssh_private_key = key.to_pem
end

chef_vault_secret "ssh" do
  data_bag 'os'
  raw_data({ "private-key" => key.to_pem })
  admins "#{ nodes },#{ bootstrap }"
  search '*:*'
  action :create_if_missing
end

chef_vault_secret "ssl" do
  data_bag 'os'
  raw_data(lazy do
              if node.run_state[:bcpc][:cert_temp] != ""
                { 'private-key' => %x[echo "#{node.run_state[:bcpc][:cert_temp]}" | openssl rsa -passin pass:temp_passwd -out /dev/stdout],
                  'certificate' => %x[echo "#{node.run_state[:bcpc][:cert_temp]}" | openssl x509] }
              else
                { 'certificate' => get_config!("ssl-certificate"),
                  'private-key' => get_config!("ssl-private-key") }
              end
            end)
  admins "#{ nodes },#{ bootstrap }"
  search '*:*'
  action :create_if_missing
end

directory "/root/.ssh" do
    owner "root"
    group "root"
    mode 00700
end

file "/root/.ssh/authorized_keys" do
    content lazy{<<EOH
################################################
#
#              Generated by Chef
#
################################################
#{get_config('ssh-public-key')}
EOH
}
    owner "root"
    group "root"
    mode 00644
end

file "/root/.ssh/id_rsa" do
    content lazy{<<EOH
################################################
#
#              Generated by Chef
#
################################################
#{get_config!('private-key','ssh','os')}
EOH
}
    owner "root"
    group "root"
    mode 00600
end

file "/etc/ssl/certs/ssl-bcpc.pem" do
    content lazy{<<EOH
################################################
#
#              Generated by Chef
#
################################################
#{get_config!('certificate','ssl','os')}
EOH
}
    owner "root"
    group "root"
    mode 00644
end

directory "/etc/ssl/private" do
    owner "root"
    group "root"
    mode 00700
end

file "/etc/ssl/private/ssl-bcpc.key" do
    content lazy{<<EOH
################################################
#
#              Generated by Chef
#
################################################
#{get_config!('private-key','ssl','os')}
EOH
}
    owner "root"
    group "root"
    mode 00600
end

